...
def show_admin_chat(initial_student_id=None):
    ...
    with tab_personal:
        ...
        if selected_id:
            ...
            st.info("💡 チャットを閉じるには、上の検索欄をクリアしてください")
    ...
    with tab_grade:
        ...
        # デバッグ
        st.write(f"🔍 DEBUG: message_sent_grade の状態: {st.session_state.get('message_sent_grade', 'None')}")
        ...
        if st.button("送信", type="primary", use_container_width=True, key=f"btn_grade_{grade}"):
            if text and text.strip():
                ...
                # デバッグ
                st.write(f"🔍 DEBUG: message_sent_grade を True に設定しました")
                st.rerun()
            else:
                st.warning("⚠️ メッセージを入力してください")
    ...
    with tab_class:
        ...
        if class_name:
            ...
            # 送信成功メッセージを表示
            if st.session_state.get("message_sent_class"):
                st.success("✅ 送信しました")
    ...
    with tab_all:
        ...
        # 送信成功メッセージを表示
        if st.session_state.get("message_sent_all"):
            st.success("✅ 送信しました")
